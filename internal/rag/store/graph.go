...
type GraphStore interface {
	...
	GetImpactContext(ctx context.Context, tenantID, projectID, filePath string) (*domain.ImpactReport, error)
	// QueryContext retrieves sub-graph or relevant context for a file
	QueryContext(ctx context.Context, tenantID, projectID, filePath string) ([]domain.CodeNode, error)
	// FindRelatedByEmbedding retrieves nodes that are semantically similar
	FindRelatedByEmbedding(ctx context.Context, tenantID, projectID string, embedding []float32, limit int) ([]domain.CodeNode, error)
	// GetFileHash retrieves the stored hash for a file node
	GetFileHash(ctx context.Context, tenantID, projectID, path string) (string, error)
	// DeleteNodesByFile removes all nodes and relations associated with a file
	DeleteNodesByFile(ctx context.Context, tenantID, projectID, path string) error
	// DeleteNodesByProject removes all nodes and relations associated with a project
	DeleteNodesByProject(ctx context.Context, tenantID, projectID string) error
}
